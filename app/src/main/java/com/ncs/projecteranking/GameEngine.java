...
public class GameEngine {
    ...
    boolean onKeyEvent(KeyEvent keyEvent) {
        ...
        switch (keyEvent.getKeyCode()) {
            ...
            case KeyEvent.KEYCODE_W: input.jump(); audio.jump(); break;
            case KeyEvent.KEYCODE_S: input.pause(); break;
    ...
    void draw(Canvas canvas) {
        ...
        canvas.drawText("||", 87, 17, paint);

        // Score text properties
        Paint paintScore = new Paint();
        paintScore.setTextSize(6);
        paintScore.setColor(Color.WHITE);
        paintScore.setTypeface(Typeface.create("Serif", Typeface.NORMAL));
        canvas.drawText("SCORE: " + String.valueOf(Score.getScoreEnd()), 67, 5, paintScore);

        // Lifes properties
        float left = 3;

        canvas.scale(0.255f, 0.255f);
        for(int i = 0; i<lifesCount ; i++) {
            left +=20;
            canvas.drawBitmap(getBitmap(80), left, 5, null);
        }

        if(bonk.ArrowShow == true) {
            // Speedboost arrows indicator
            canvas.scale(1.37f, 1.37f);
            canvas.drawBitmap(getBitmap(64), 145.8f, 0,null);
            canvas.drawBitmap(getBitmap(65), 162, 0,null);
    ...
    private void showPauseDialog() {
        dialogBuilder.setTitle("PAUSA.")
                .setMessage("Pulsa CONTINUAR per seguir jugant.")
